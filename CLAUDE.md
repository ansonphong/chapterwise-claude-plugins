...
- **2026-08-05** — v2.10.2. Report filenames stopped stuttering. `Chrysalis - Dome Show Script.codex.yaml` produced `chrysalis---dome-show-script-…` because the report slug replaced spaces with hyphens and then dropped the remaining punctuation one character at a time, so `" - "` became three hyphens. `analysis_writer.analysis_file_id` had the identical bug and was fixed in 2.5.0; the sibling in `analysis_report.py` was missed, and the two slug the same manuscript name for two different files. Extracted as `report_slug()` with the collapse rule and a test asserting the two functions agree. Surfaced by running `/analysis immersive_design` end to end on a real 36-beat dome script — every unit-test fixture was named `show.codex.yaml`, so nothing in 316 tests had a space in it.
